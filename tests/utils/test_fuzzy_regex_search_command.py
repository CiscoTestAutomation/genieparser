...
class TestFuzzyRegexSearchCommand(unittest.TestCase):
    def test_search_normal_arguments(self):
        ...
        for command, expected_source in parser_data.items(): 
            ...
            results = _fuzzy_regex_search_command(search, False)
            is_found = False
    ...
    def test_special_command(self):
        results = _fuzzy_regex_search_command('/dna/intent/api/v1/interface', False)
        self.assertTrue(len(results), 1)
        self.assertEqual(results[0][0], '/dna/intent/api/v1/interface')

        results = _fuzzy_regex_search_command('/dna/intent/api/v1/interface/argument', False)
        self.assertTrue(len(results), 1)
        self.assertEqual(results[0][0], '/dna/intent/api/v1/interface/{interface}')
        self.assertEqual(results[0][2], {'interface': 'argument'})

        results = _fuzzy_regex_search_command(r'\/dna\/intent\/api\/v1\/interface\/argument', True)
        self.assertTrue(len(results), 1)
        self.assertEqual(results[0][0], '/dna/intent/api/v1/interface/{interface}')
        self.assertEqual(results[0][2], {'interface': 'argument'})

        results = _fuzzy_regex_search_command(r'\/dna\/intent\/api\/v1\/interface', True)
        self.assertTrue(len(results), 1)
        self.assertEqual(results[0][0], '/dna/intent/api/v1/interface')


if __name__ == '__main__':
    #print(_fuzzy_regex_search_command('/dna/intent/api/v1/interface', False))

    #results = _fuzzy_regex_search_command('\/dna\/intent\/api\/v1\/interface\/argument', True)
    #exit()
    # unittest.main() 
    # exit()
    # print(_fuzzy_regex_search_command('\/dna\/intent\/api\/v1\/interface\/apcd', True)[0][0])
    # exit()

    for command, expected_source in parser_data.items(): 
        arguments = re.findall('{(.*?)}', command)
        search = re.sub('{.*?}', 'machester', command)
        expected_kwargs = {argument:'argument' for argument in arguments}
    ...
    #_fuzzy_regex_search_command('show access-lists wjosf', False)
    ...
    #     expected_kwargs = {argument:'argument' for argument in arguments}
    #     results = _fuzzy_regex_search_command(command, False)
    #     if len(results) != 1:
    #         print("ERROR ", command)
    #         count += 1
    #         break
    ...
    #print(_fuzzy_regex_search_command('/mgmt/tm/auth/radius', False))
    ...
    # # OR operator
    # for w in _fuzzy_regex_search_command('sh b .* blue .* [n|p] d'):
    #     print(w[0])
    #     print(w[2])
    #     print('========')
