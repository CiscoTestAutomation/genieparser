...
class ShowBgpProcessVrfAll(ShowBgpProcessVrfAllSchema):
    ...
    def xml(self):
        ...
        for line in out.splitlines():
            ...
            m = p48.match(line)
            if m:
                export_rt_list.append(m.groupdict()['export_rt'])
                if 'export_rt_list' not in parsed_dict['vrf'][vrf_name]\
                    ['address_family'][address_family]:
                    parsed_dict['vrf'][vrf_name]['address_family']\
            ...
            m = p49.match(line)
            if m:
                import_rt_list.append(m.groupdict()['import_rt'])
                if 'import_rt_list' not in parsed_dict['vrf'][vrf_name]\
                    ['address_family'][address_family]:
                    parsed_dict['vrf'][vrf_name]['address_family']\
            ...
            p50 = re.compile(r'^\s*\<importdefault_prefixcount\>'
                              '(?P<value>([0-9])+)'
