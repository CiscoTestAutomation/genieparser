...
###################################################
# unit test for show ip cef <prefix> internal
####################################################
class TestShowIpCefInternal(unittest.TestCase):
    """unit test for show ip cef <ip> internal"""

    device = Device(name='aDevice')
    empty_output = {'execute.return_value': ''}

    golden_output_1 = {'execute.return_value': '''\
        show ip cef 27.86.198.239 internal

        Load for five secs: 1%/0%; one minute: 0%; five minutes: 0%

        Time source is NTP, *20:26:24.572 JST Mon May 25 2020
        ...
        feature space:
            ...
            dflt local label info: global/27 [0x3]

            sr local label info: global/16073 [0x1B]

                contains path extension list

                dflt disposition chain 0x7FECC1EE0B28

                label 30284

                FRR Primary

                    <primary: TAG adj out of GigabitEthernet0/3/6, addr 106.162.196.213>

                dflt label switch chain 0x7FECC1EE0B28
                ...
                sr disposition chain 0x7FECC1EE0240
                ...
                sr label switch chain 0x7FECC1EE0240
        ...
        subblocks:
            ...
            non-eos chain loadinfo 7FECBFE026E0, per-session, flags 0111, 58 locks

        ifnums:

            GigabitEthernet0/3/6(17): 106.162.196.213

            MPLS-SR-Tunnel1(29)

        path list 7FECC02E6490, 19 locks, per-destination, flags 0x4D [shble, hvsh, rif, hwcn]

            path 7FECC01D7270, share 1/1, type attached nexthop, for IPv4, flags [has-rpr]

            MPLS short path extensions: [rib | prfmfi | lblmrg | srlbl] MOI flags = 0x2020 label 30284

            nexthop 106.162.196.213 GigabitEthernet0/3/6 label [30284|16073]-(local:27), IP adj out of GigabitEthernet0/3/6, addr 106.162.196.213 7FECB7D7F048

                repair: attached-nexthop 106.187.14.241 MPLS-SR-Tunnel1 (7FECC01D04C0)

            path 7FECC01D04C0, share 1/1, type attached nexthop, for IPv4, flags [rpr, rpr-only]

            MPLS short path extensions: [rib | prfmfi | lblmrg | srlbl] MOI flags = 0x1 label 16073

            nexthop 106.187.14.241 MPLS-SR-Tunnel1 label 16073-(local:16073), repair, IP midchain out of MPLS-SR-Tunnel1 7FECBF3B19E0

        output chain:

            label [30284|16073]-(local:27)

            FRR Primary (0x80007FECC01CF778)

            <primary: TAG adj out of GigabitEthernet0/3/6, addr 106.162.196.213 7FECBF3B3650>

            <repair:  TAG midchain out of MPLS-SR-Tunnel1 7FECBF3B17B0

                        label 15023

                        TAG adj out of GigabitEthernet0/3/7, addr 106.162.196.217 7FECBF3B1C10>
           '''}
