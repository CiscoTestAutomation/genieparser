...
class ShowDeviceTrackingDatabase(ShowDeviceTrackingDatabaseSchema):
    ...
    def cli(self, vlan_id=None, address=None, output=None):
        ...
        # ARP 192.168.20.11 aabb.0000.0191 Et0/1 2020 0005 6mn STALE try 0 1710 s try 0
        device_info_capture_database = re.compile(
            ...
            r"(?P<pref_level_code>\d+)\s+(?P<age>\S+)\s+(?P<state>\S+)\s+(?P<time_left>(try\s\d\s\d+\s(s|ms)(\stry\s\d)?)|(N/A)|(\(.+\))|(\d+.*)|(\d+\s(s|ms)\stry\d))$")
