        ...
        for line in output.splitlines():
            ...
            if m:
                ...
                if group['minor']:
                    ...
                    for key in ['minor', 'major', 'critical', 'shutdown', 'unit']:
                        fin_dict['threshold'][key] = int(group[key]) if key != 'unit' else group[key]
                continue
...
class ShowEnvironmentAll(ShowEnvironmentAllSchema):
    ...
    def cli(self, include='', output=None):
        ...
        p6 = re.compile(r'(?P<ps_slot>PS\S+)\s+(?P<model_no>\S+)\s+(?P<type>\S+)\s+(?P<capacity>\S+(\s+\S+)?)\s+(?P<status>\S+)\s+(?P<fan_1_state>\S+)\s+(?P<fan_2_state>\S+)$')

        # PS Current Configuration Mode : Combined
        # PS Current Operating State    : Combined
        # Power supplies currently active    : 1
        # Power supplies currently available : 1
        p7 = re.compile(r'(PS|Power supplies)\s+(?P<ps_key>.+)\s+:\s+(?P<ps_value>\S+)')
        ...
        for line in output.splitlines():
            ...
            if m:
                group = m.groupdict()
                sensor_dict = ret_dict.setdefault('sensor_list', {}).setdefault(group['sensor_list'], {})
                continue
            ...
            if m:
                ...
                ps_slot_dict = ps_dict.setdefault('slot', {}).setdefault(ps_slot, {})
            ...
            if m:
                group = m.groupdict()
                fantray_key = group['fantray_key'].strip().lower().replace(' ', '_')
                ...
                if 'fantray' == fantray_key:
                    ret_dict.setdefault('fantray', {}).setdefault('status', fantray_value)
                else:
                    ret_dict.setdefault('fantray', {}).setdefault(fantray_key, fantray_value)
