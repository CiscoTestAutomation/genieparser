...
class ShowCryptoSession(ShowCryptoSessionSchema):

    """ Parser for "show crypto session" 
    ...
    def cli(self,detail_arg='',output=None):
        if output is None:
            out = self.device.execute(self.cli_command.format(detail=detail_arg))
        else:
            out = output
