...
class ShowFabricApSummary(ShowFabricApSummarySchema):
    ...
    def cli(self, output=None):
        ...
        #AP0029.C2DE.65B0        2      1852I     0029.c2de.65b0  0029.c2df.d9e0  default location    US    112.201.2.152   Registered   
        p2 = re.compile(r"^(?P<ap_name>\S+)\s+(?P<slots>\d+)\s+(?P<ap_model>\S+)\s+(?P<ethernet_mac>\S+)\s+(?P<radio_mac>\S+)\s+(?P<location>.*)\s+(?P<country>\S+)\s+(?P<ip_address>\d+\.\d+\.\d+\.\d+)\s+(?P<state>(Registered))")
