...
class ShowHardwareLed(ShowHardwareLedSchema):
    ...
    def cli(self, output=None): 
        ...
        p10 = re.compile(r'^FANTRAY\s+(?P<fantray_id>\d+)\s+STATUS:\s+(?P<fantray_status>\w+)$')

        # FANTRAY 1 BEACON: OFF
        p11 = re.compile(r'^FANTRAY\s+(?P<fantray_id>\d+)\s+BEACON:\s+(?P<fantray_beacon>\w+)$')

        # ACTIVE SSD: OFF
        p12 = re.compile(r'^ACTIVE\s+SSD:\s+(?P<active_ssd>\w+)$')

        # STANDBY SSD: OFF
        p13 = re.compile(r'^STANDBY\s+SSD:\s+(?P<standby_ssd>\w+)$')

        # CHASSIS FANTRAY STATUS: RED
        p14 = re.compile(r'^CHASSIS\s+FANTRAY\s+STATUS:\s+(?P<chassis_fantray_status>\w+)$')
        ...
        for line in output.splitlines():
            ...
            if m:
                group = m.groupdict()
                fantray_id = int(group['fantray_id'])
                fantray_status = group['fantray_status']
                fantray_dict = root_dict.setdefault('fantray', {}).setdefault(fantray_id, {})
                fantray_dict['fantray_status'] = fantray_status
                continue
            ...
            if m:
                group = m.groupdict()
                fantray_id = int(group['fantray_id'])
                fantray_beacon = group['fantray_beacon']
                fantray_dict = root_dict.setdefault('fantray', {}).setdefault(fantray_id, {})
                fantray_dict['fantray_beacon'] = fantray_beacon
                continue

            # ACTIVE SSD:  OFF
            m = p12.match(line)
            if m:
                root_dict['active_ssd'] = m.groupdict()['active_ssd']
                continue
        
            # STANDBY SSD: OFF
            m = p13.match(line)
            if m:
                root_dict['standby_ssd'] = m.groupdict()['standby_ssd']
                continue

            # CHASSIS FANTRAY STATUS: RED
            m = p14.match(line)
            if m:
                root_dict['chassis_fantray_status'] = m.groupdict()['chassis_fantray_status']
                continue
