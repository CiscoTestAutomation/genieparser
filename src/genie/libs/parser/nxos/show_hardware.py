...
class ShowHardwareInternalTctrlUsdDpllState(ShowHardwareInternalTctrlUsdDpllStateSchema):
    ...
    def cli(self, output=None):
        ...
        # ------------------------ INST:0 DPLL 0 ------------------------
        p0 = re.compile(r'\-+ INST:(?P<inst>\d+)+ DPLL (?P<dpll>\d+)')

        # dpll_mon_status_0 (0x0118): 0x02: in Holdover
        p1 = re.compile(r'^[\s\S]+(?P<dpll_mon_status>\(\w+\)\:\s\w+\:\s\w+\s\w+)$')

        # lock: No
        p2 = re.compile(r'^lock:+ (?P<lock>\w+)$')

        # ho: Yes
        p3 = re.compile(r'^ho:+ (?P<ho>\w+)$')

        # step_time in progress No
        p4 = re.compile(r'^step_time in progress+ (?P<step_time_in_progress>\w+)$')

        # flhit No
        p5 = re.compile(r'^flhit+ (?P<flhit>\w+)$')

        # pslhit No
        p6 = re.compile(r'^pslhit+ (?P<pslhit>\w+)$')

        # dpll 0 mode Automatic mode(3) refId INVALID(15)
        p7 = re.compile(r'dpll+ \d+ +mode+ (?P<mode>\w+) \w+\(\d+\) refId+ (?P<refId>\w+)')

        # NCO Assist Pair HW lock status: Ref Failed(4)
        p8 = re.compile(r'^NCO Assist Pair HW lock status:(?P<nco_assist_pair_hw_lock_status>[\s\w\(\\d)]+)$')

        # PSL 885 Bandwidth 7 Var BW 79 PullInRange 0
        p9 = re.compile(r'PSL (?P<psl>\d+) \w+ (?P<bandwidth>\d+) \w+ \w+ (?P<var_bw>\d+) \w+ (?P<pullinrange>\d+)')

        # ZL30XXX_REF0P(0) : 7743 NA
        p10 = re.compile(r'(?P<refclk>\w+\(\d+\)) \: (?P<measured_freq>\d+) (?P<freq_err>(\w+|\-\d+\.\d+))')
