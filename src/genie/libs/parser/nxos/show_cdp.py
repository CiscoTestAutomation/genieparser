...
class ShowCdpNeighborsDetail(ShowCdpNeighborsDetailSchema):
    ...
    def cli(self, output=None):
        ...
        #System Name:swor96
        system_name_re = re.compile(r'System\s*Name\s*:\s*(?P<system_name>\S+)')
        ...
        physical_location_re = re.compile(r'Physical\s*Location\s*:\s*(?P<physical_location>\S*)')
        ...
        interface_address_re = re.compile(r'Interface\s*address\s*\(\w+\)\s*\:\s*')
        ...
        ipv4_address_re = re.compile(r'\S*IPv4\s*Address:\s*(?P<ip_address>\S*)')
        ...
        for line in out.splitlines():
            ...
            if result:
                ...
                devices_dict[index_device]['physical_location'] = ''
                devices_dict[index_device]['system_name'] = ''
                devices_dict[index_device]['management_addresses'] = {}
                devices_dict[index_device]['interface_addresses'] = {}

                continue

            result = system_name_re.match(line)
            if result:
                devices_dict[index_device]['system_name'] = result.group('system_name')
                continue
            ...
            if result:
                devices_dict[index_device]['physical_location'] = result.group('physical_location')
