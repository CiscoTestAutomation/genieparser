...
class ShowInterfacesTerse(ShowInterfacesTerseSchema):
    ...
    cli_command = [
        'show interfaces terse', 'show interfaces {interface} terse'
    ]
    ...
    def cli(self, interface=None, output=None):
        ...
        # fxp0                    up    up
        p1 = re.compile(
            r'^(?P<interface>\S+) +(?P<admin_state>\w+) +(?P<link_state>\w+) *'
        ...
        #                                             10.11.11.11         --> 0/0
        p2 = re.compile(
            r'^((?P<protocol>\S+) +)?(?P<local>((\d+\.[\d\.\/]+)|(\w+\:[\w\:\/]+)|(0x\d+))+)'
            ' *(([\-\>]+) *(?P<remote>[\w\.\:\/]+))?$')
        #                                    multiservice
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                if groups['protocol']:
                    ...
                    pro_dict = intf_dict.setdefault('protocol', {}).setdefault(
                        groups['protocol'], {})
                if groups['local']:
            ...
            if m:
                ...
                try:
                    ...
                    continue
                pro_dict = intf_dict.setdefault('protocol', {}).setdefault(
                    protocol, {}).setdefault(groups['local'], {})
                pro_dict['local'] = groups['local']
                if groups['remote']:
                    pro_dict['remote'] = groups['remote']
                continue
            ...
            if m:
                ...
                pro_dict = intf_dict.setdefault('protocol',
                                                {}).setdefault(protocol, {})
                continue
        return ret_dict
...
class ShowInterfacesDescriptions(ShowInterfacesDescriptionsSchema):
    ...
    def cli(self, interface=None, output=None):
        ...
        p2 = re.compile(
            r'^(?P<name>\S+) +(?P<admin_status>\S+) +(?P<oper_status>\S+) +(?P<description>\S+)$'
        )
        ...
        for line in out.splitlines():
            ...
            if m:
                group = m.groupdict()
                entry_list = ret_dict.setdefault("interface-information",
                                                 {}).setdefault(
                                                     "physical-interface", [])
                entry = {}
...
class ShowInterfacesSchema(MetaParser):
    ...
    def verify_physical_interface_list(value):
        ...
        def verify_logical_interface_list(value):
            ...
            def verify_address_family_list(value):
                ...
                def verify_interface_address_list(value):
                    ...
                    if not isinstance(value, list) and not isinstance(
                            value, dict):
                        raise SchemaError(
                            'interface-address is not a list/dict')

                    interface_address_schema = Schema({
                        Optional("ifa-broadcast"):
                        str,
                        Optional("ifa-destination"):
                        str,
                        Optional("generation"):
                        str,
                        "ifa-flags": {
                ...
                af_schema = Schema({
                    ...
                    Use(verify_interface_address_list),
                    Optional("intf-curr-cnt"):
                    str,
                    Optional("intf-dropcnt"):
                    str,
                    Optional("intf-unresolved-cnt"):
                    str,
                    Optional("generation"):
                    str,
                    Optional("route-table"):
                    str,
                    Optional("max-local-cache"):
                    str,
                    Optional("maximum-labels"):
                    str,
                    Optional("mtu"):
                    str,
                    Optional("new-hold-limit"):
                    str,
            ...
            def verify_if_distribution_list_information_list(value):
                ...
                if not isinstance(value, list) and not isinstance(value, dict):
                    raise SchemaError(
                        'if-distribution-list-information is not a list/dict')
            ...
            def verify_lag_lacp_info_list(value):
                ...
                lag_lacp_info_list_schema = Schema({
                    Optional("lacp-port-key"):
                    str,
                    Optional("lacp-port-number"):
                    str,
                    Optional("lacp-port-priority"):
                    str,
                    Optional("lacp-role"):
                    str,
                    Optional("lacp-sys-priority"):
                    str,
                    Optional("lacp-system-id"):
                    str,
                    Optional("name"):
                    str
                })
            ...
            def verify_lag_lacp_statistics_list(value):
                ...
                lag_lacp_statistics_list_schema = Schema({
                    Optional("illegal-rx-packets"):
                    str,
                    Optional("lacp-rx-packets"):
                    str,
                    Optional("lacp-tx-packets"):
                    str,
                    Optional("name"):
                    str,
                    Optional("unknown-rx-packets"):
                    str
                })
                # Validate each dictionary in list
                if isinstance(value, dict):
            ...
            def verify_lag_marker_list(value):
                ...
                lag_marker_list_schema = Schema({
                    ...
                    Optional("lacp-tx-packets"):
                    str,
                    Optional("marker-response-tx-packets"):
                    str,
                    Optional("marker-rx-packets"):
                    str,
                    Optional("name"):
                    str,
                    Optional("unknown-rx-packets"):
            ...
            l_i_schema = Schema({
                ...
                Use(verify_address_family_list),
                ...
                Optional("lag-traffic-statistics"): {
                    ...
                    Optional("if-distribution-list-information"):
                    Use(verify_if_distribution_list_information_list),
                    ...
                    Optional("lag-bundle"):
                    Use(verify_lag_bundle_list),
                    Optional("lag-lacp-info"):
                    Use(verify_lag_lacp_info_list),
                    Optional("lag-lacp-statistics"):
                    Use(verify_lag_lacp_statistics_list),
                    Optional("lag-link"):
                    Use(verify_lag_link_list),
                    Optional("lag-marker"):
                    Use(verify_lag_marker_list),
                },
                "local-index":
                str,
                Optional("logical-interface-bandwidth"):
                str,
                "name":
                str,
                Optional("description"):
                str,
                Optional("policer-overhead"):
                str,
                Optional("snmp-index"):
                str,
        ...
        def verify_queue_num_forwarding_class_name_map_list(value):
            ...
            if not isinstance(value, list):
                raise SchemaError(
                    'queue_num_forwarding_class_map is not a list')
        ...
        physical_interface_schema = Schema({
            Optional("down-hold-time"):
            str,
            Optional("up-hold-time"):
            str,
            Optional("statistics-cleared"):
            str,
            ...
            Optional("clocking"):
            str,
            Optional("current-physical-address"):
            str,
            Optional("description"):
            str,
            Optional("eth-switch-error"):
            str,
            ...
            Optional("hardware-physical-address"):
            str,
            ...
            Optional("if-flow-control"):
            str,
            Optional("if-media-flags"): {
                "ifmf-none": bool
            },
            Optional("if-remote-fault"):
            str,
            Optional("if-type"):
            str,
            ...
            Optional("interface-transmit-statistics"):
            str,
            Optional("l2pt-error"):
            str,
            Optional("ld-pdu-error"):
            str,
            Optional("link-level-type"):
            str,
            Optional("link-type"):
            str,
            Optional("link-mode"):
            ...
            Use(verify_logical_interface_list),
            Optional("loopback"):
            str,
            Optional("minimum-links-in-aggregate"):
            str,
            Optional("minimum-bandwidth-in-aggregate"):
            str,
            ...
            "name":
            str,
            Optional("oper-status"):
            str,
            Optional("pad-to-minimum-frame-size"):
            str,
            ...
            Optional("cos-information"): {
                Optional("cos-stream-information"): {
                    ...
                    Use(verify_cos_queue_configuration)
                }
            },
            ...
            Optional("queue-num-forwarding-class-name-map"):
            Use(verify_queue_num_forwarding_class_name_map_list)
        })
        # Validate each dictionary in list
        for item in value:
...
class ShowInterfaces(ShowInterfacesSchema):
    ...
    def cli(self, interface=None, output=None):
        ...
        p4 = re.compile(
            r'^(Type: +\S+, )?Link-level +type: +(?P<link_level_type>\S+), +MTU: +(?P<mtu>\S+)(, +MRU: +(?P<mru>\d+))?(, +(?P<sonet_mode>\S+) +mode)?(, +Link-mode: +(?P<link_mode>\S+))?(, +Speed: +(?P<speed>\S+))?(, +BPDU +Error: +(?P<bpdu_error>\S+))?(, +Loopback: +(?P<loopback>\S+))?,$'
        )
        ...
        p4_2 = re.compile(
            r'^Link-level +type: +(?P<link_level_type>\S+), +MTU: +(?P<mtu>\S+)'
        ...
        p5_1 = re.compile(
            r'^((Ethernet-Switching +Error: +(?P<eth_switch_error>[^\s,]+))|(BPDU +Error: +(?P<bpdu_error>[^\s,]+)))(, +)?(MAC-REWRITE +Error: +[^\s,]+)?(, +)?(Loopback: +(?P<loopback>[^\s,]+))(, +)?'
        )
        ...
        p6 = re.compile(
            r'^Source +filtering: +(?P<source_filtering>\S+), +Flow +control: +(?P<if_flow_control>\S+)(, +Auto-negotiation: +(?P<if_auto_negotiation>\S+), +Remote +fault: +(?P<if_remote_fault>\S+))?$'
        )
        ...
        p7_1 = re.compile(
            r'^Minimum +links +needed: +(?P<minimum_links_in_aggregate>\d+), +Minimum +bandwidth +needed: +(?P<minimum_bandwidth_in_aggregate>\S+)$'
        )
        ...
        p9 = re.compile(
            r'^Interface +flags:( +(?P<hardware_down>Hardware-Down))? +'
        ...
        p11 = re.compile(
            r'^CoS +queues +: +(?P<physical_interface_cos_hw_max_queues>\d+) +'
        ...
        p12 = re.compile(
            r'^Current +address: +(?P<current_physical_address>\S+), +'
        ...
        p22_1 = re.compile(
            r'^FEC +Uncorrected +Errors +(?P<fec_nccw_count>\d+)$')
        p22_2 = re.compile(
            r'^FEC +Corrected +Errors +Rate +(?P<fec_ccw_error_rate>\d+)$')
        p22_3 = re.compile(
            r'^FEC +Uncorrected +Errors +Rate +(?P<fec_nccw_error_rate>\d+)$')
        ...
        p25 = re.compile(
            r'^Flags: +(?P<iff_up>(\S+|Hardware-Down Device-Down))'
        ...
        p30 = re.compile(
            r'^Max +nh +cache: +(?P<max_local_cache>\d+), +New +hold +nh +limit: +(?P<new_hold_limit>\d+), Curr +nh +cnt: +(?P<intf_curr_cnt>\d+),( +Curr +new +hold +cnt: +(?P<intf_unresolved_cnt>\d+), +NH +drop +cnt: +(?P<intf_dropcnt>\d+))?$'
        )

        # Curr new hold cnt: 0, NH drop cnt: 0
        p30_1 = re.compile(
            r'^Curr +new +hold +cnt: +(?P<intf_unresolved_cnt>\d+), +NH +drop +cnt: +(?P<intf_dropcnt>\d+)$'
        )
        ...
        p33 = re.compile(
            r'^Destination: +(?P<ifa_destination>\S+), +Local: +(?P<ifa_local>\S+)(, +Broadcast: +(?P<ifa_broadcast>\S+))?(, +Generation: +(?P<generation>\S+))?$'
        )
        ...
        p40 = re.compile(
            r'^Damping +: +half-life: +\d+ +sec, +max-suppress: +'
        ...
        p44_1 = re.compile(
            r'^Carrier +transitions: +(?P<carrier_transitions>\d+),'
        ...
        p52 = re.compile(
            r'^MAC +control +frames +(?P<input_mac_control_frames>\d+) +'
            r'(?P<output_mac_control_frames>\d+)$')

        # MAC pause frames                         0                0
        p53 = re.compile(
            r'^MAC +pause +frames +(?P<input_mac_pause_frames>\d+) +'
            r'(?P<output_mac_pause_frames>\d+)$')

        # Oversized frames                         0
        p54 = re.compile(
            r'^Oversized +frames +(?P<input_oversized_frames>\d+)$')
        ...
        p58 = re.compile(
            r'^VLAN +tagged +frames +(?P<input_vlan_tagged_frames>\d+)$')
        ...
        # Total errors                             0                0
        p60 = re.compile(
            r'^Total +errors +(?P<input_total_errors>\d+) +(?P<output_total_errors>\d+)$'
        )

        # Label-switched interface (LSI) traffic statistics:
        p61 = re.compile(
            r'^Label-switched +interface +\(LSI\) +traffic +statistics:$')

        # Ingress queues: 8 supported, 4 in use
        # Egress queues: 8 supported, 4 in use
        p62 = re.compile(
            r'^((?P<intf_cos_queue_type>(Ingress|Egress) +queues)): +(?P<intf_cos_num_queues_supported>\d+) +supported, +(?P<intf_cos_num_queues_in_use>\d+) +in +use$'
        )

        # 0                                0                    0                    0
        p63 = re.compile(
            r'^(?P<queue_number>\d+) +(?P<queue_counters_queued_packets>\d+) +'
            r'(?P<queue_counters_trans_packets>\d+) +(?P<drop_packets>\d+)$')

        # Hold-times     : Up 0 ms, Down 0 ms
        p64 = re.compile(
            r'^Hold-times +: +Up +(?P<up_hold_time>\d+) +ms, +Down +(?P<down_hold_time>\d+) +ms$'
        )

        # Statistics last cleared: 2020-10-14 13:18:51 EST (00:12:30 ago)
        p65 = re.compile(
            r'^Statistics +last +cleared: +(?P<statistics_cleared>[\S\s]+)$')
        ...
        # 3                   network-control
        p66 = re.compile(
            r'^(?P<queue_number>\d+) +(?P<forwarding_class_name>best-effort|expedited-forwarding|assured-forwarding|network-control)$'
        )
        ...
        p69 = re.compile(
            r'^Input +packet +rejects +(?P<input_reject_count>\d+)$')

        # Input DA rejects                         0
        p70 = re.compile(
            r'^Input +DA +rejects +(?P<input_reject_da_count>\d+)$')

        # Input SA rejects                         0
        p71 = re.compile(
            r'^Input +SA +rejects +(?P<input_reject_sa_count>\d+)$')
        ...
        p73 = re.compile(
            r'^Output +packet +pad +count +(?P<output_packet_pad_count>\d+)$')

        # Output packet error count                                 0
        p74 = re.compile(
            r'^Output +packet +error +count +(?P<output_packet_error_count>\d+)$'
        )

        # CAM destination filters: 0, CAM source filters: 0
        p75 = re.compile(
            r'^CAM +destination +filters: (?P<cam_destination_filter_count>\d+), +CAM +source +filters: (?P<cam_source_filter_count>\d+)$'
        )

        # Destination slot: 0 (0x00)
        p76 = re.compile(
            r'^Destination +slot: +(?P<destination_slot>\d+) +(?P<destination_mask>\S+)$'
        )

        # 0 best-effort            95     9500000000    95              0      low    none
        p77 = re.compile(
            r'^(?P<cos_queue_number>\d+) +(?P<cos_queue_forwarding_class>\S+) +(?P<cos_queue_bandwidth>\d+) +(?P<cos_queue_bandwidth_bps>\d+) +(?P<cos_queue_buffer>\d+) +(?P<cos_queue_buffer_bytes>\d+) +(?P<cos_queue_priority>\w+) +(?P<cos_queue_limit>\w+)$'
        )
        ...
        p80 = re.compile(
            r'^Policer: +Input: +(?P<policer_input>\S+)(, +Output: +(?P<policer_output>\S+))?$'
        )
        ...
        # Output:            16          0          1188            0
        p82 = re.compile(
            r'^(?P<in_out>(Input\s*)|(Output)):\s+(?P<packets>\d+)\s+(?P<pps>\d+)\s+(?P<bytes>\d+)\s+(?P<bps>\d+)$'
        )
        ...
        # Adaptive Updates:          0
        p83 = re.compile(
            r'^(?P<adaptive>Adaptive\s+(Adjusts|Scans|Updates))\s*:\s+(?P<adaptive_value>\d+)$'
        )

        # Aggregate member links: 2
        p84 = re.compile(
            r'^Aggregate\s+member\s+links:\s+(?P<aggregate_member_count>\d+)$')

        # LACP info:        Role     System             System       Port     Port    Port
        # LACP Statistics:       LACP Rx     LACP Tx   Unknown Rx   Illegal Rx
        # Marker Statistics:   Marker Rx     Resp Tx   Unknown Rx   Illegal Rx
        p85 = re.compile(
            r'^(?P<lacp_flag>(LACP info)|(LACP Statistics)|(Marker Statistics)):\s+.+$'
        )

        # ge-0/0/6.0     Actor        127  2c:6b:f5:d6:f8:c0        127        2       1
        # ge-0/0/6.0   Partner        127  2c:6b:f5:18:ef:c0        127        2       1
        p86 = re.compile(
            r'^(?P<name>\S+)\s+(?P<lacp_role>\S+)\s+(?P<lacp_sys_priority>\d+)\s+(?P<lacp_system_id>\S+)\s+(?P<lacp_port_priority>\d+)\s+(?P<lacp_port_number>\d+)\s+(?P<lacp_port_key>\d+)$'
        )

        # For LACP Statistics
        # ge-0/0/6.0                 0           0            0            0
        p87 = re.compile(
            r'(?P<name>\S+)\s+(?P<lacp_rx_packets>\d+)\s+(?P<lacp_tx_packets>\d+)\s+(?P<unknown_rx_packets>\d+)\s+(?P<illegal_rx_packets>\d+)$'
        )

        # For Marker Statistics
        # ge-0/0/6.0                 0           0            0            0
        p88 = re.compile(
            r'(?P<name>\S+)\s+(?P<marker_rx_packets>\d+)\s+(?P<marker_response_tx_packets>\d+)\s+(?P<unknown_rx_packets>\d+)\s+(?P<illegal_rx_packets>\d+)$'
        )
        ...
        # Standby         Down
        p89 = re.compile(
            r'^(?P<list_type>(Primary)|(Backup)|(Standby))\s+(?P<list_status>(Active)|(Down))$'
        )

        # ge-0/0/7        Up
        p90 = re.compile(
            r'^(?P<if_child_name>\S+)\s+(?P<if_status>(Up)|(Down))$')
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                interface_info_dict = ret_dict.setdefault(
                    'interface-information', {})
                physical_interface_list = interface_info_dict.setdefault(
                    'physical-interface', [])
                physical_interface_dict = {}
                physical_interface_dict.update({'name': group['name']})
                admin_status = group['admin_status']
                admin_status_dict = physical_interface_dict.setdefault(
                    'admin-status', {})
            ...
            if m:
                group = m.groupdict()
                physical_interface_dict.update({
                    k.replace('_', '-'): v
            ...
            if m:
                ...
                if_device_flags_dict = physical_interface_dict.setdefault(
                    'if-device-flags', {})
            ...
            if m:
                group = m.groupdict()
                if_config_flags_dict = physical_interface_dict.setdefault(
                    'if-config-flags', {})
                ...
                if group['internal_flags']:
                    if_config_flags_dict.update(
                        {'internal-flags': group['internal_flags']})
                continue
            ...
            if m:
                ...
                if_media_flags_dict = physical_interface_dict.setdefault(
                    'if-media-flags', {})
            ...
            if m:
                group = m.groupdict()
                cos_dict = physical_interface_dict.setdefault(
                    'physical-interface-cos-information', {})
                cos_dict.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
            ...
            if m:
                group = m.groupdict()
                intf_flapped_dict = physical_interface_dict.setdefault(
                    'interface-flapped', {})
            ...
            if m:
                group = m.groupdict()
                traffic_statistics_dict = traffic_statistics_dict.setdefault(
                    'ipv6-transit-statistics', {})
            ...
            if m:
                ...
                group = m.groupdict()
                traffic_statistics_dict = physical_interface_dict.setdefault(
            ...
            if m:
                ...
                if statistics_type == 'physical':
                    ...
                    traffic_statistics_dict = logical_interface_dict.setdefault(
                        'transit-traffic-statistics', {})
            ...
            if m:
                if statistics_type == 'physical':
                    ...
                    traffic_statistics_dict = logical_interface_dict.setdefault(
                        'traffic-statistics', {})
                group = m.groupdict()
                traffic_statistics_dict.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                ...
                if statistics_type == 'dropped_stp_state':
                    traffic_statistics_dict.update({
                        'stp-{}-dropped'.format(k.replace('_', '-')): v
                        for k, v in group.items() if v is not None
                    })
                else:
                    traffic_statistics_dict.update({
            ...
            if m:
                ...
                active_alarms_dict = physical_interface_dict.setdefault(
                    'active-alarms', {})
            ...
            if m:
                ...
                active_defects_dict = physical_interface_dict.setdefault(
                    'active-defects', {})
            ...
            if m:
                group = m.groupdict()
                statistics_dict = physical_interface_dict.setdefault(
                    'ethernet-pcs-statistics', {})
                continue
            ...
            if m:
                ...
                physical_interface_dict.update(
                    {'interface-transmit-statistics': inft_transmit})
                continue
            ...
            if m:
                ...
                logical_interface_list = physical_interface_dict.setdefault(
                    'logical-interface', [])
                logical_interface_dict.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
            ...
            if m:
                group = m.groupdict()
                if_config_flags_dict = logical_interface_dict.setdefault(
                    'if-config-flags', {})
            ...
            if m:
                group = m.groupdict()
                address_family_list = logical_interface_dict.setdefault(
                    'address-family', [])
                address_family_dict = {
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
            ...
            if m:
                group = m.groupdict()
                address_family_flags_dict = address_family_dict.setdefault(
                    'address-family-flags', {})
                for flag in group['flags'].split(','):
                    if "encapsulation" in flag.lower():
                        value = flag.split(":")[-1].strip().lower()
                        address_family_flags_dict.update(
                            {"ifff-encapsulation": value})
                    else:
            ...
            if m:
                ...
                ifa_flags_dict = interface_address_dict.setdefault(
                    'ifa-flags', {})
                ...
                if af_check:
                    if isinstance(af_check, dict):
                        ...
                        interface_address_list = address_family_dict[
                            'interface-address']
                    ...
                    address_family_dict.setdefault('interface-address',
                                                   interface_address_dict)
                continue
            ...
            m = p41.match(line)
            if m:
                input_error_list_dict = physical_interface_dict.setdefault(
                    'input-error-list', {})
                continue
            ...
            m = p42.match(line)
            if m:
                output_error_list_dict = physical_interface_dict.setdefault(
                    'output-error-list', {})
                continue
            ...
            if m:
                group = m.groupdict()
                ethernet_mac_statistics.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            m = p45_1.match(line)
            if m:
                ethernet_mac_statistics = physical_interface_dict.setdefault(
            ...
            if m:
                ...
                if queue_name == 'Egress queues':
                    cos_short_summary_dict = physical_interface_dict.setdefault(
                        'queue-counters',
            ...
            if m and queue_name:
                group = m.groupdict()
                if queue_name == 'Egress queues':
                    queue_list = physical_interface_dict.setdefault(
                        'queue-counters', {}).setdefault('queue', [])
                elif queue_name == 'Ingress queues':
                    queue_list = physical_interface_dict.setdefault(
                        'ingress-queue-counters', {}).setdefault('queue', [])
                queue_dict = {}
                queue_dict.update({'queue-number': group['queue_number']})
                queue_dict.update({
                    'queue-counters-queued-packets':
                    group['queue_counters_queued_packets']
                })
                queue_dict.update({
                    'queue-counters-trans-packets':
                    group['queue_counters_trans_packets']
                })
                queue_dict.update({
            ...
            if m:
                ...
                if 'queue-counters' in physical_interface_dict:
                    queue_list = physical_interface_dict.setdefault(
                        'queue-counters', {}).setdefault('queue', [])
                    queue_list[queue_number].update({
                        k.replace('_', '-'): v
                        for k, v in group.items() if v is not None
                    })
            ...
            m = p67.match(line)
            if m:
                ethernet_filter_statistics = physical_interface_dict.setdefault(
            ...
            if m:
                group = m.groupdict()
                pfe_information = physical_interface_dict.setdefault(
                    'pfe-information', {})
                pfe_information.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                ...
                cos_queue_configuration_list = cos_stream_information_dict.setdefault(
                    'cos-queue-configuration', [])
                cos_queue_configuration_dict = {
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                }
                cos_queue_configuration_list.append(
                    cos_queue_configuration_dict)
                continue
            ...
            if m:
                ...
                cos_stream_information_dict = physical_interface_dict.setdefault("cos-information", {}). \
                    setdefault("cos-stream-information", {})
                cos_stream_information_dict.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                group = m.groupdict()
                policer_information_dict = address_family_dict.setdefault(
                    'policer-information', {})
            ...
            if m:
                group = m.groupdict()
                lag_traffic_dict = logical_interface_dict.setdefault(
                    'lag-traffic-statistics', {})
            ...
            if m:
                ...
                in_out_dict = {
                    "{iod}-{k}".format(iod=in_out_direction.rstrip().lower(),
                                       k=k): v
                    for k, v in group.items() if v is not None
                }
            ...
            if m:
                group = m.groupdict()
                lag_traffic_dict.setdefault(
                    'lag-adaptive-statistics',
                    {}).setdefault(group['adaptive'].lower().replace(' ', '-'),
                                   group['adaptive_value'])
                continue
            ...
            if m:
                ...
                lag_traffic_dict['aggregate-member-info'] = {
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                }
                continue
            ...
            if m and lacp_flag == 'LACP Statistics':
                ...
                lag_traffic_dict['lag-lacp-statistics'].append({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m and lacp_flag == 'Marker Statistics':
                ...
                lag_traffic_dict['lag-marker'].append({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                ...
                lag_traffic_dict['if-distribution-list-information'].append(
                    if_dist_dict)
                continue
            ...
            if m and if_dist_dict:
                group = m.groupdict()
                lag_traffic_dict['if-distribution-list-information'][
                    -1].setdefault('if-list', [])
                if_list_dict = {
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                }
                lag_traffic_dict['if-distribution-list-information'][-1][
                    'if-list'].append(if_list_dict)
                continue
...
class ShowInterfacesExtensive(ShowInterfaces):
    cli_command = [
        'show interfaces extensive', 'show interfaces {interface} extensive'
    ]
...
class ShowInterfacesStatisticsSchema(MetaParser):
    ...
    def validate_physical_interface_list(value):
        ...
        def validate_logical_interface_list(value):
            ...
            def validate_address_family_list(value):
                ...
                address_family_schema = Schema({
                    ...
                    Optional("interface-address"):
                    Use(validate_interface_address_list),
                })
            ...
            logical_interface_schema = Schema({
                ...
                Optional("filter-information"):
                str,
                Optional("logical-interface-zone-name"):
                str,
                ...
                Optional("address-family"):
                Use(validate_address_family_list),
            })
        ...
        physical_interface_schema = Schema({
            ...
            Optional("input-error-count"):
            str,
            Optional("output-error-count"):
            str,
            Optional("active-alarms"): {
                "interface-alarms": {
                    Optional("alarm-not-present"): bool,
            ...
            str,
            Optional("logical-interface"):
            Use(validate_logical_interface_list)
        })
...
class ShowInterfacesStatistics(ShowInterfacesStatisticsSchema):
    ...
    cli_command = [
        "show interfaces statistics", "show interfaces statistics {interface}"
    ]
    ...
    def cli(self, interface=None, output=None):
        ...
        # Link-level type: Ethernet, MTU: 1514, Link-mode: Full-duplex, Speed: 1000mbps,
        p3 = re.compile(
            r'^Link-level +type: +(?P<link_level_type>[^\s,]+), +'
        ...
        p13 = re.compile(
            r'^Statistics last cleared: +(?P<statistics_cleared>.*)$')
        ...
        # Security: Zone: trust
        p24 = re.compile(
            r'^Security: +Zone: (?P<logical_interface_zone_name>\S+)$')

        # Allowed host-inbound traffic : dhcp http https ssh telnet
        p25 = re.compile(
            r'^Allowed +host-inbound +traffic *: +'
        ...
        # Protocol inet, MTU: 1500
        p26 = re.compile(
            r'^Protocol +(?P<address_family_name>[^\s,]+), +MTU: +(?P<mtu>\S+)$'
        )

        # Flags: Sendbcast-pkt-to-re, Is-Primary
        p27 = re.compile(
            r'^Flags: +(?P<ifff_sendbcast_pkt_to_re>Sendbcast-pkt-to-re)'
        ...
        p29 = re.compile(
            r'^(Destination: +(?P<ifa_destination>[^\s,]+))?(, +)?'
        ...
        for line in out.splitlines():
            ...
            if m:
                group = m.groupdict()
                if_device_flags = physical_interface_dict.setdefault(
                    'if-device-flags', {})
                if_device_flags.update({
                    k.replace('_', '-'): True
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                group = m.groupdict()
                if_config_flags = physical_interface_dict.setdefault(
                    'if-config-flags', {})
                if_config_flags.update({
                    k.replace('_', '-'): True
                    for k, v in group.items()
                    if v is not None and k != "internal_flags"
                })
            ...
            if m:
                group = m.groupdict()
                if_media_flags = physical_interface_dict.setdefault(
                    'if-media-flags', {})
                if_media_flags.update({
                    k.replace('_', '-'): True
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                group = m.groupdict()
                phys_cos_info = physical_interface_dict.setdefault(
                    'physical-interface-cos-information', {})
                phys_cos_info.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                group = m.groupdict()
                traffic_stats = physical_interface_dict.setdefault(
                    'traffic-statistics', {})
            ...
            if m:
                group = m.groupdict()
                active_alarm = physical_interface_dict.setdefault(
                    'active-alarms', {})
                interface_alarm = active_alarm.setdefault(
                    'interface-alarms', {})
                interface_alarm.update({
                    k.replace('_', '-'): True
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                ...
                active_alarm = physical_interface_dict.setdefault(
                    'active-defects', {})
                interface_defect = active_alarm.setdefault(
                    'interface-alarms', {})
                interface_defect.update({
                    k.replace('_', '-'): True
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                group = m.groupdict()
                if_config_flags = logical_interface_dict.setdefault(
                    'if-config-flags', {})
                if_config_flags.update({
                    k.replace('_', '-'): True
                    for k, v in group.items() if v is not None
                    and k not in ["encapsulation", "internal_flags"]
                })
            ...
            if m:
                group = m.groupdict()
                traffic_stats = logical_interface_dict.setdefault(
                    'traffic-statistics', {})
                traffic_stats.update({
                    k.replace('_', '-'): v
                    for k, v in group.items() if v is not None
            ...
            if m:
                group = m.groupdict()
                allowed_in_traffic = logical_interface_dict.setdefault(
                    'allowed-host-inbound-traffic', {})
                allowed_in_traffic.update({
                    k.replace('_', '-'): True
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                group = m.groupdict()
                address_family_flags = address_family_dict.setdefault(
                    'address-family-flags', {})
                address_family_flags.update({
                    k.replace('_', '-'): True
                    for k, v in group.items() if v is not None
                })
                continue
            ...
            if m:
                group = m.groupdict()
                interface_address_list = address_family_dict.setdefault(
                    'interface-address', [])
                interface_address_dict = {}
                ...
                if 'flags' in group and group['flags']:
                    ...
                    ifa_flags.update({
                        "ifaf-current-{}".format(f.split('-')[-1].lower()):
                        True
                        for f in flags
                    })
                interface_address_list.append(interface_address_dict)
            ...
            if m:
                ...
                if group['ifa_destination']:
                    interface_address_dict.update(
                        {'ifa-destination': group['ifa_destination']})
                if group['ifa_local']:
                    interface_address_dict.update(
                        {'ifa-local': group['ifa_local']})
                if group['ifa_broadcast']:
                    interface_address_dict.update(
                        {'ifa-broadcast': group['ifa_broadcast']})
                continue
...
class ShowInterfacesPolicersInterfaceSchema(MetaParser):
    ...
    def validate_logical_interface_list(value):
        ...
        logical_interface_schema = Schema({
            ...
            str,
            "policer-information":
            Use(ShowInterfacesPolicersInterface.
                validate_policer_information_list)
        })
        # Validate each dictionary in list
        for item in value:
            logical_interface_schema.validate(item)
    ...
    def validate_physical_interface_list(value):
        ...
        physical_interface_schema = Schema({
            ...
            str,
            "logical-interface":
            Use(ShowInterfacesPolicersInterface.validate_logical_interface_list
                ),
            "name":
            str,
            "oper-status":
...
class ShowInterfacesPolicersInterface(ShowInterfacesPolicersInterfaceSchema):
    ...
    def cli(self, interface=None, output=None):
        ...
        # ge-0/0/2        up    up
        p1 = re.compile(
            r'^(?P<interface>[a-zA-Z\-\d\/]+)((?P<physical_interface_value>[\.\d]+))? +(?P<admin>\w+) +(?:(?P<link>\w+))?$'
        )

        # inet  GE_1M-ge-0/0/2.0-log_int-i GE_1M-ge-0/0/2.0-log_int-o
        # multiservice __default_arp_policer__
        p2 = re.compile(
            r'^(?P<policer_family>\w+) +(?P<policer_input>\S+)( +((?P<policer_output>\S+)))?$'
        )
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                if exists:
                    ...
                    logical_interface_dict['name'] = group[
                        'interface'] + group['physical_interface_value']
                    logical_interface_dict['admin-status'] = group['admin']
                    logical_interface_dict['oper-status'] = group['link']

                    policer_information_list = []
                    logical_interface_dict[
                        "policer-information"] = policer_information_list
                    logical_interface_list.append(logical_interface_dict)

                    interface_policer_info_dict[
                        'logical-interface'] = logical_interface_list
                    ...
                    interface_policer_info_dict['admin-status'] = group[
                        'admin']
            ...
            if m:
                group = m.groupdict()
                policer_information_dict = {}
                policer_information_dict['policer-family'] = group[
                    'policer_family']
                policer_information_dict['policer-input'] = group[
                    'policer_input']

                for group_key, group_value in group.items():
                    if group_key == "policer_output":
                        if group_value != None:
                            policer_information_dict['policer-output'] = group[
                                'policer_output']
...
class ShowInterfacesQueue(ShowInterfacesQueueSchema):
    ...
    def cli(self, interface=None, output=None):
        ...
        p5 = re.compile(
            r"^(?P<intf_cos_queue_type>Egress queues): (?P<intf_cos_num_queues_supported>\S+) supported, "
        ...
        p9 = re.compile(
            r"^(?P<name>RED-dropped packets) +: +(?P<counts>\S+) +(?P<rates>\S+) +pps$"
        )
        #              Low                 :                     0                     0 pps
        #              Medium-low          :                     0                     0 pps
        #              Medium-high         :                     0                     0 pps
        ...
        p10 = re.compile(
            r"^(?P<name>RED-dropped bytes) +: +(?P<counts>\S+) +(?P<rates>\S+) +bps$"
        )
        #              Low                 :                     0                     0 bps
        #              Medium-low          :                     0                     0 bps
        #              Medium-high         :                     0                     0 bps
        ...
        for line in out.splitlines():
            ...
            m = p3.match(line)
            if m:
                physical_interface_dict['description'] = m.groupdict(
                )['description']
                continue
            ...
            if m:
                ...
                physical_interface_dict['queue-counters']['queue'].append(
                    current_queue_dict)
                continue
            ...
            if m:
                ...
                if red_dropped_bytes:
                    if name == 'Low':
                        current_queue_dict[
                            'queue-counters-red-bytes-low'] = counts
                        current_queue_dict[
                            'queue-counters-red-bytes-rate-low'] = rates
                    elif name == 'Medium-low':
                        current_queue_dict[
                            'queue-counters-red-bytes-medium-low'] = counts
                        current_queue_dict[
                            'queue-counters-red-bytes-rate-medium-low'] = rates
                    elif name == 'Medium-high':
                        current_queue_dict[
                            'queue-counters-red-bytes-medium-high'] = counts
                        current_queue_dict[
                            'queue-counters-red-bytes-rate-medium-high'] = rates
                    elif name == 'High':
                        current_queue_dict[
                            'queue-counters-red-bytes-high'] = counts
                        current_queue_dict[
                            'queue-counters-red-bytes-rate-high'] = rates

                # RED-dropped packets
                elif red_dropped_packets:
                    if name == 'Low':
                        current_queue_dict[
                            'queue-counters-red-packets-low'] = counts
                        current_queue_dict[
                            'queue-counters-red-packets-rate-low'] = rates
                    elif name == 'Medium-low':
                        current_queue_dict[
                            'queue-counters-red-packets-medium-low'] = counts
                        current_queue_dict[
                            'queue-counters-red-packets-rate-medium-low'] = rates
                    elif name == 'Medium-high':
                        current_queue_dict[
                            'queue-counters-red-packets-medium-high'] = counts
                        current_queue_dict[
                            'queue-counters-red-packets-rate-medium-high'] = rates
                    elif name == 'High':
                        current_queue_dict[
                            'queue-counters-red-packets-high'] = counts
                        current_queue_dict[
                            'queue-counters-red-packets-rate-high'] = rates

                # Transmitted
                elif transmitted:
                    if name == 'Packets':
                        current_queue_dict[
                            'queue-counters-trans-packets'] = counts
                        current_queue_dict[
                            'queue-counters-trans-packets-rate'] = rates
                    elif name == 'Bytes':
                        current_queue_dict[
                            'queue-counters-trans-bytes'] = counts
                        current_queue_dict[
                            'queue-counters-trans-bytes-rate'] = rates
                    elif name == 'Tail-dropped packets':
                        current_queue_dict[
                            'queue-counters-tail-drop-packets'] = counts
                        current_queue_dict[
                            'queue-counters-tail-drop-packets-rate'] = rates
                    elif name == 'RL-dropped packets':
                        current_queue_dict[
                            'queue-counters-rl-drop-packets'] = counts
                        current_queue_dict[
                            'queue-counters-rl-drop-packets-rate'] = rates

                # Queued
                else:
                    if name == 'Packets':
                        current_queue_dict[
                            'queue-counters-queued-packets'] = counts
                        current_queue_dict[
                            'queue-counters-queued-packets-rate'] = rates
                    elif name == 'Bytes':
                        current_queue_dict[
                            'queue-counters-queued-bytes'] = counts
                        current_queue_dict[
                            'queue-counters-queued-bytes-rate'] = rates
            ...
            if m:
                ...
                if group['name'] == 'RED-dropped packets':
                    current_queue_dict['queue-counters-red-packets'] = group[
                        'counts']
                    current_queue_dict[
                        'queue-counters-red-packets-rate'] = group['rates']
                continue
            ...
            if m:
                ...
                if group['name'] == 'RED-dropped bytes':
                    current_queue_dict['queue-counters-red-bytes'] = group[
                        'counts']
                    current_queue_dict[
                        'queue-counters-red-bytes-rate'] = group['rates']
                continue
        return ret_dict
