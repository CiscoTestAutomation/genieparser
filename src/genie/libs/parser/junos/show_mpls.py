...
class ShowMPLSLSPNameDetail(ShowMPLSLSPNameDetailSchema):
    ...
    def cli(self, name, output=None):
        ...
        p1 = re.compile(r'^(?P<session_type>\S+) +LSP: +(?P<count>\d+) +sessions$')
        ...
        p6 = re.compile(r'^Suggested +label +received: +(?P<suggested_label_in>[^\s,]+), +'
                        r'Suggested +label +sent: +(?P<suggested_label_out>\S+)$')

        # Recovery label received: -, Recovery label sent: 44
        p7 = re.compile(r'^Recovery +label +received: +(?P<recovery_label_in>[^\s,]+), +'
                        r'Recovery +label +sent: +(?P<recovery_label_out>\S+)$')

        # Resv style: 1 FF, Label in: 46, Label out: 44
        p8 = re.compile(r'^Resv +style: +(?P<rsb_count>\d+) +(?P<resv_style>[^\s,]+), +'
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                if rsvp_session.get('record-route'):
                    ...
                    rsvp_session.update(
                        {'destination-address': re.findall(r'[\d\.]+', line)[0]}
                    )
                continue
