...
class ShowSystemBuffer(ShowSystemBufferSchema):
    ...
    def cli(self, node_id=None, output=None):
        ...
        p10 = re.compile(
            r"^(?P<sfbuf_requests_denied>\S+) +requests +for +sfbufs" r" +denied$"
        )

        # 0 requests for sfbufs delayed
        p11 = re.compile(
            r"^(?P<sfbuf_requests_delayed>\S+) +requests +for" r" +sfbufs +delayed$"
        )

        # 0 requests for I/O initiated by sendfile
        p12 = re.compile(
            r"^(?P<io_initiated>\S+) +requests +for +I/O +initiated" r" +by +sendfile$"
        )
        ...
        statistics_schema = Schema(
            {
                ...
                Optional("icmp"): {
                    ...
                    "echo-drops-with-broadcast-or-multicast-destinaton-address": str,
                    "errors-not-generated-because-old-message-was-icmp": str,
                    "histogram": Use(icmp_histogram_list),
                    "message-responses-generated": str,
                    "messages-less-than-the-minimum-length": str,
                    "messages-with-bad-checksum": str,
                    "messages-with-bad-code-fields": str,
        ...
        for line in out.splitlines():
            ...
            if self.state == "udp":
                ...
                if m:
                    ...
                    if key == "datagrams_received":
                        ...
                        key == "broadcast/multicast_datagrams_dropped_due_to_no_socket"
                    ):
                        entry[
            ...
            if self.state == "ah":
                ...
                if m:
                    ...
                    if key == "packets_shorter_than_header_shows":
                        ...
                        entry[
                            "packets-dropped-due-to-bad-authentication-length"
                        ] = value
            ...
            if self.state == "esp":
                ...
                if m:
                    ...
                    if key == "packets_shorter_than_header_shows":
                        ...
                        entry[
                            "esp-packets-dropped-as-bad-authentication-detected"
                        ] = value
                    elif key == "possible_replay_packets_detected":
            ...
            if self.state == "ipcomp":
                ...
                if m:
                    ...
                    if key == "packets_shorter_than_header_shows":
                        ...
                        entry[
                            "ipcomp-packets-dropped-as-protocol-not-supported"
                        ] = value
                    elif key == "packets_dropped_no_TDB":
                        ...
                        entry[
                            "ipcomp-packets-dropped-as-larger-than-ip-maxpacket"
                        ] = value
                    elif key == "packets_blocked_due_to_policy":
            ...
            if self.state == "raw_if":
                ...
                if m:
                    ...
                    key = key.replace(" ", "_")
                    value = group["number_value"]
                    entry = ret_dict["statistics"][0].setdefault("raw-interface", {})
            ...
            if self.state == "arp":
                ...
                if m:
                    ...
                    if key == "datagrams_received":
                        ...
                        key == "arp_response_packets_are_rejected_on_mace_icl_interface"
                    ):
                        entry[
            ...
            if self.state == "ip6":
                ...
                if m:
                    ...
                    header_type_list = (
                        ret_dict["statistics"][0]
                        .setdefault("ip6", {})
                        .setdefault("header-type", [])
                    )
                    entry = {
                        "header-for-source-address-selection": group[
                            "header_for_source_address_selection"
                        ]
                    }
            ...
            if self.state == "icmp6":
                ...
                if m:
                    ...
                    histo = entry.setdefault(self.icmp6_histogram + "-histogram", {})
                ...
                if m:
                    ...
                    if key == "Calls_to_icmp_error":
                        ...
                        key == "Errors_not_generated_because_old_message_was_icmp_error"
                    ):
                        entry[
                        ...
                        if self.icmp6_histogram == "output":
                            entry["output-histogram"][
                                "router-solicitation-icmp6-packets"
                            ] = value
                        elif self.icmp6_histogram == "input":
                            entry["input-histogram"][
                                "router-solicitation-icmp6-packets"
                        ...
                        if self.icmp6_histogram == "output":
                            entry["output-histogram"][
                                "router-advertisement-icmp6-packets"
                            ] = value
                        elif self.icmp6_histogram == "input":
                            entry["input-histogram"][
                                "router-advertisement-icmp6-packets"
                        ...
                        key == "interface-restricted_dad_packets_from_same_node_dropped"
                    ):
            ...
            if self.state == "pfkey":
                ...
                if m:
                    ...
                    if key == "Requests_sent_from_userland":
                        ...
                        if (
                            not "outgoing-messages-with-memory-allocation-failure"
                            in entry
                        ):
                            entry[
                                "outgoing-messages-with-memory-allocation-failure"
                            ] = value
                        else:
                            entry[
                                "incoming-messages-with-memory-allocation-failure"
                            ] = value
            ...
            if self.state == "ttp":
                ...
                if m:
                    ...
                    if key == "Packets_sent":
                        ...
                        entry[
                            "input-packets-for-which-route-lookup-is-bypassed"
                        ] = value
                    elif (
                        key
            ...
            if self.state == "ethoamlfm":
                ...
                if m:
                    ...
                    key = key.replace(" ", "_")
                    value = group["number_value"]
                    entry = ret_dict["statistics"][0].setdefault("ethoamlfm", {})
            ...
            if self.state == "ethoamcfm":
                ...
                if m:
                    ...
                    key = key.replace(" ", "_")
                    value = group["number_value"]
                    entry = ret_dict["statistics"][0].setdefault("ethoamcfm", {})
            ...
            if self.state == "vpls":
                ...
                if m:
                    ...
                    if key == "Total_packets_received":
                        ...
                        key == "Learning_requests_while_learning_disabled_on_interface"
                    ):
                        entry[
