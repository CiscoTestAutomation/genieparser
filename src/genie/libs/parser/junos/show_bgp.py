        ...
        entry_schema = Schema(
            {
                ...
                Optional("bgp-output-queue"): Use(validate_bgp_output_queue),
                ...
                Optional("bgp-error"): Use(validate_bgp_error),
...
class ShowBgpNeighbor(ShowBgpNeighborSchema):
    ...
    def cli(self, output=None):
        ...
        p3 = re.compile(r'^Group: +(?P<peer_group>\S+) +Routing-Instance: +(?P<peer_cfg_rti>\S+)$')
        # Forwarding routing-instance: master
        p4 = re.compile(r'^Forwarding +routing-instance: +(?P<peer_fwd_rti>\S+)$')
        # Type: Internal    State: Active       (route reflector client)Flags: <>
        p5 = re.compile(r'^Type: +(?P<peer_type>\S+) +State: +(?P<peer_state>\S+) +\(route +reflector +client\)Flags: +<(?P<peer_flags>\S*)>$')
        # Last State: Idle          Last Event: Start
        p6  = re.compile(r'^Last +State: +(?P<last_state>\S+) +Last +Event: +(?P<last_event>\S+)')
        # Last Error: None
        p7 = re.compile(r'^Last +Error: +(?P<last_error>[\S\s]+)$')
        # Export: [ v4_Kentik_NO-DEFAULT ] Import: [ 11 ]
        p8 = re.compile(r'^Export: +\[ +(?P<export_policy>\S+) +\] +Import: +\[ +(?P<import_policy>\S+) +\]$')
        ...
        p15 = re.compile(r'^Address +families +configured: +(?P<address_families>[\S\s]+)$')
        # Type: External    State: Established    Flags: <Sync>
        p16 = re.compile(r'^Type: +(?P<peer_type>\S+) +State: +(?P<peer_state>\S+) +Flags: +<(?P<peer_flags>\S*)>$')
        ...
        p21 = re.compile(r'^I\/O +Session +Thread: +(?P<iosession_thread_name>\S+) +State: +(?P<iosession_state>\S+)$')
        # BFD: disabled, down
        p22 = re.compile(r'^BFD: +(?P<bfd_configuration_state>\S+), +(?P<bfd_operational_state>\S+)$')
        # NLRI for restart configured on peer: inet-unicast inet-labeled-unicast
        p23 = re.compile(r'^NLRI +for +restart +configured +on +peer: +(?P<peer_restart_nlri_configured>[\S\s]+)$')
        # NLRI advertised by peer: inet-unicast inet-labeled-unicast
        p24 = re.compile(r'^NLRI +advertised +by +peer: +(?P<nlri_type_peer>[\s\S]+)$')
        # NLRI for this session: inet-unicast inet-labeled-unicast
        p25 = re.compile(r'^NLRI +for +this +session: +(?P<nlri_type_session>[\s\S]+)$')
        # Peer supports Refresh capability (2)
        p26 = re.compile(r'^Peer +supports +Refresh +capability +\((?P<peer_refresh_capability>\d+)\)$')
        # Stale routes from peer are kept for: 300
        p27 = re.compile(r'^Stale +routes +from +peer +are +kept +for: +(?P<peer_stale_route_time_configured>\d+)$')
        # Peer does not support Restarter functionality
        p28 = re.compile(r'^Peer +does +not +support +Restarter +functionality$')
        # Restart flag received from the peer: Notification
        p29 = re.compile(r'^Restart +flag +received +from +the +peer: +(?P<peer_restart_flags_received>\S+)$')
        # NLRI that restart is negotiated for: inet-unicast inet-labeled-unicast
        p30 = re.compile(r'^NLRI +that +restart +is +negotiated +for: +(?P<peer_restart_nlri_negotiated>[\S\s]+)$')
        # NLRI of received end-of-rib markers: inet-unicast inet-labeled-unicast
        p31 = re.compile(r'^NLRI +of +received +end-of-rib +markers: +(?P<peer_end_of_rib_received>[\S\s]+)$')
        # NLRI of all end-of-rib markers sent: inet-unicast inet-labeled-unicast
        p32 = re.compile(r'^NLRI +of +all +end-of-rib +markers +sent: +(?P<peer_end_of_rib_sent>[\S\s]+)$')
        # Peer does not support LLGR Restarter functionality
        p33 = re.compile(r'^Peer +does +not +support +LLGR +Restarter +functionality$')
        # Peer supports 4 byte AS extension (peer-as 65151)
        p34 = re.compile(r'^Peer +supports +4 +byte +AS +extension +\(peer-as +(?P<peer_4byte_as_capability_advertised>\S+)\)$')
        # Peer does not support Addpath
        p35 = re.compile(r'^Peer +does +not +support +Addpath$')
        # NLRI(s) enabled for color nexthop resolution: inet-unicast
        p36 = re.compile(r'^NLRI\(s\) +enabled +for +color +nexthop +resolution: +(?P<local_ext_nh_color_nlri>\S+)$')
        ...
        p38 = re.compile(r'^Entropy +label: +(?P<entropy_label>\S+); +next +hop +validation: +(?P<entropy_label_no_next_hop_validation>\S+)$')
        #     Local entropy label capability: Yes; stitching capability: Yes
        p39 = re.compile(r'^Local +entropy +label +capability: +(?P<entropy_label_capability>\S+); +stitching +capability: +(?P<entropy_label_stitching_capability>\S+)$')
        ...
        p44 = re.compile(r'^Received +prefixes: +(?P<received_prefix_count>\d+)$')
        #     Accepted prefixes:            684
        p45 = re.compile(r'^Accepted +prefixes: +(?P<accepted_prefix_count>\d+)$')
        #     Suppressed due to damping:    0
        p46 = re.compile(r'^Suppressed +due +to +damping: +(?P<suppressed_prefix_count>\d+)$')
        #     Advertised prefixes:          0
        p47 = re.compile(r'^Advertised +prefixes: +(?P<advertised_prefix_count>\d+)$')
        # Last traffic (seconds): Received 3    Sent 3    Checked 1999164
        p48 = re.compile(r'^Last +traffic +\(seconds\): +Received +(?P<last_received>\S+) +Sent +(?P<last_sent>\S+) +Checked +(?P<last_checked>\S+)$')
        # Input messages:  Total 280022 Updates 61419   Refreshes 0     Octets 7137084
        p49 = re.compile(r'^Input +messages: +Total +(?P<input_messages>\S+) +Updates +(?P<input_updates>\S+) +Refreshes +(?P<input_refreshes>\S+) +Octets +(?P<input_octets>\S+)$')
        # Output messages: Total 221176 Updates 0       Refreshes 0     Octets 4202359
        p50 = re.compile(r'^Output +messages: +Total +(?P<output_messages>\S+) +Updates +(?P<output_updates>\S+) +Refreshes +(?P<output_refreshes>\S+) +Octets +(?P<output_octets>\S+)$')
        # Output Queue[1]: 0            (inet.0, inet-unicast)
        p51 = re.compile(r'^Output +Queue\[(?P<number>\d+)\]: +(?P<count>\d+) +\((?P<table_name>\S+), +(?P<rib_adv_nlri>\S+)\)$')
        ...
        for line in out.splitlines():
            ...
            if m:
                group = m.groupdict()
                entry_list = ret_dict.setdefault("bgp-information", {}).setdefault("bgp-peer", [])
                entry = {}
                for key, value in group.items():
                    key = key.replace('_', '-')
                    entry[key] = value
            ...
            if m:
                group = m.groupdict()
                entry = ret_dict["bgp-information"]["bgp-peer"][-1]["bgp-rib"][-1]
                for key, value in group.items():
                    key = key.replace('_', '-')
                    entry[key] = value
                continue
