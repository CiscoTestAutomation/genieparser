...
class Ping(PingSchema):
    ...
    def cli(
            ...
            if addr and count:
                if ttl and wait:
                    cmd = self.cli_command[2].format(
                        addr=addr, count=count, ttl=ttl, wait=wait
                    )
                elif source and size and tos:
                    cmd = self.cli_command[5].format(
                        addr=addr, source=source, size=size, count=count, tos=tos
                    )
                    ...
                    cmd = self.cli_command[3].format(
                        addr=addr, source=source, count=count
                    )
                elif size:
                    cmd = self.cli_command[6].format(addr=addr, size=size, count=count)
                else:
        ...
        p3 = re.compile(
            r"^(?P<send>\d+) +packets +transmitted, +"
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                ping_dict.update(
                    {
                        k.replace("_", "-"): (int(v) if v.isdigit() else v)
