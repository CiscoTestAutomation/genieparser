...
class TestFuzzyRegexSearchCommand(unittest.TestCase):
    ...
    def test_search_normal_arguments(self):
        ...
        self.assertIsNotNone(common.parser_data)

        for command in common.parser_data:
    ...
    def test_search_normal_arguments_with_regex(self):
        ...
        for command in common.parser_data:
            ...
            for result in results:
                found_command, found_class, kwargs = result
                # found_class will always be None because we are not providing
                # abstract tokens
                if command == found_command and found_class is None:
                    is_found = True
