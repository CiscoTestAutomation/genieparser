    ...
    schema = {
        "neighbors": {
            Any(): {
                ...
                "last_write_reset": {
                    "last_write_reset": str,
                    "attempted": int,
                    "written": int,
                },
                "second_last_write_reset": {
                    "second_last_write_reset": str,
                    "attempted": int,
                    "written": int,
                },
                "last_write_pulse_rcvd": str,
                "last_full_not_set_pulse_count": int,
                "last_write_pulse_rcvd_reset": str,
                "socket_io": bool,
                "socket_read": bool,
                "socket_write": bool,
                "last_write_thread_event_reset": str,
                "second_last_write_thread_event_reset": str,
                "last_ka_expiry": str,
                "second_last_ka_expiry": str,
                "last_ka_error": str,
                "ka_not_sent": str,
                "last_ka_start": str,
                "second_last_ka_start": str,
                "precedence": str,
                "non_stop_routing": str,
                "multi_protocol_capability": str,
                "neighbor_capabilities": {
                    "route_refresh": str,
                    "four_octets_as": str,
                    "ipv4_unicast": str,
                    "ipv6_labeled_unicast": str,
                    "l2vpn_evpn": str,
                },
                "received_msg": int,
                ...
                "sent_in_queue": int,
                "minimum_time_between_advertisement_runs": int,
                "inbound_msg_logging": "enabled",
                "inbound_msg_buffered": int,
                "outbound_msg_logging": "enabled",
                "outbound_msg_buffered": int,
                "address_family": {
                    Any(): {
                        "bgp_neighbor_version": int,
                        "update_group": float,
                        "filter_group": float,
                        Optional("extended_nexthop_encoding"): str,
                        "route_refresh_request_received": int,
                        "route_refresh_request_sent": int,
                        "accepted_prefixes": int,
                        "bestpaths": int,
                        "exact_no_prefixes_denied": int,
                        "cummulative_no_prefixes_denied": int,
                        Optional("no_policy"): int,
                        Optional("failed_rt_match"): int,
                        Optional("by_orf_policy"): int,
                        Optional("by_policy"): int,
                        "prefix_advertised": int,
                        "prefix_suppressed": int,
                        "prefix_withdrawn": int,
                        "maximum_prefixes_allowed": int,
                        "warning_msg_threshold": str,
                        ...
                        "outstanding_version_objects_refresh": int,
                        "additional_paths_operation": str,
                    },
                },
...
class ShowBgpL2vpnEvpnNeighbors(ShowBgpL2vpnEvpnNeighborsSchema):
    ...
    def cli(self, neighbor='', output=None):
        ...
        p1 = re.compile(r'^BGP +neighbor +is +(?P<neighbor>\S+)$')

        #  Remote AS 65001, local AS 65001, internal link
        p2 = re.compile(r'^Remote +AS +(?P<remote_as>\d+), +local +AS +(?P<local_as>\d+), '
                         '+(?P<link>\w+) +link$')

        #  Remote router ID 192.168.99.11
        p3 = re.compile(r'^Remote +router +ID +(?P<router_id>\S+)$')

        #   BGP state = Established, up for 2w5d
        p4 = re.compile(r'^(BGP|Session) +state += +(?P<bgp_state>\S+)'
                         '(?:, +(?P<state>up|down) +for +(?P<time>\S+))?$')

        #   NSR State: None
        p5 = re.compile(r'^NSR +State: +(?P<nsr_state>\S+)$')

        #   Last read 00:00:09, Last read before reset 00:00:00
        p6 = re.compile(r'^Last +read +(?P<last_read>\S+), '
                         '+Last +read +before +reset +(?P<last_read_reset>\S+)$')

        #   Hold time is 180, keepalive interval is 60 seconds
        p7 = re.compile(r'^Hold +time +is +(?P<hold_time>\d+), +keepalive +interval '
                         '+is +(?P<keepalive>\d+) +seconds$')

        #   Configured hold time: 180, keepalive: 60, min acceptable hold time: 3
        p8 = re.compile(r'^Configured +hold +time: +(?P<configured_hold_time>\d+), '
                         '+keepalive: +(?P<configured_keepalive>\d+), +min +acceptable '
                         '+hold +time: +(?P<min_accept_hold_time>\d+)$')

        #   Last write 00:00:09, attempted 19, written 19
        p9 = re.compile(r'^Last +write +(?P<last_write>\S+), +attempted +(?P<attempted>\d+), '
                         '+written +(?P<written>\d+)$')

        #   Second last write 00:01:09, attempted 19, written 19
        p10 = re.compile(r'^Second +last +write +(?P<second_last_write>\S+), '
                          '+attempted +(?P<attempted>\d+), +written +(?P<written>\d+)$')

        #   Last write before reset 00:00:00, attempted 0, written 0
        p11 = re.compile(r'^Last +write +before +reset +(?P<last_write_reset>\S+), '
                          '+attempted +(?P<attempted>\d+), +written +(?P<written>\d+)$')

        #   Second last write before reset 00:00:00, attempted 0, written 0
        p12 = re.compile(r'^Second +last +write +before +reset +(?P<second_last_write_reset>\S+), '
                          '+attempted +(?P<attempted>\d+), +written +(?P<written>\d+)$')

        #   Last write pulse rcvd  Aug 13 04:20:04.721 last full not set pulse count 55044
        p13 = re.compile(r'^Last +write +pulse +rcvd +(?P<time>[\S\s]+) '
                          '+last +full +not +set +pulse +count +(?P<count>\d+)$')

        #   Last write pulse rcvd before reset 00:00:00
        p14 = re.compile(r'^Last +write +pulse +rcvd +before +reset +(?P<reset>\S+)$')

        #   Socket not armed for io, armed for read, armed for write
        p15 = re.compile(r'^Socket +(?P<socket_io>[\w\s]+) +for +io, '
                          '+(?P<socket_read>[\w\s]+) +for +read, '
                          '+(?P<socket_write>[\w\s]+) +for +write$')

        #   Last write thread event before reset 00:00:00, second last 00:00:00
        p16 = re.compile(r'^Last +write +thread +event +before +reset +(?P<last>\S+), '
                          '+second +last +(?P<second_last>\S+)$')

        #   Last KA expiry before reset 00:00:00, second last 00:00:00
        p17 = re.compile(r'^Last +KA +expiry +before +reset +(?P<last>\S+), '
                          '+second +last +(?P<second_last>\S+)$')

        #   Last KA error before reset 00:00:00, KA not sent 00:00:00
        p18 = re.compile(r'^Last +KA +error +before +reset +(?P<last>\S+), '
                          '+KA +not +sent +(?P<not_sent>\S+)$')

        #   Last KA start before reset 00:00:00, second last 00:00:00
        p19 = re.compile(r'^Last +KA +start +before +reset +(?P<last>\S+), '
                          '+second +last +(?P<second_last>\S+)$')

        #   Precedence: internet
        p20 = re.compile(r'^Precedence: +(?P<precedence>\w+)$')

        #   Non-stop routing is enabled
        p21 = re.compile(r'^Non-stop +routing +is +(?P<non_stop_routing>\w+)$')

        #   Multi-protocol capability received
        p22 = re.compile(r'^Multi-protocol +capability +(?P<multi_protocol_capability>\S+)$')

        #   Neighbor capabilities:
        p23 = re.compile(r'^Neighbor +capabilities:$')

        #     Route refresh: advertised (old + new) and received (old + new)
        p24 = re.compile(r'^Route +refresh: +(?P<route_refresh>[\S\s]+)$')

        #     4-byte AS: advertised and received
        p25 = re.compile(r'^4-byte +AS: +(?P<four_octets_as>[\S\s]+)$')

        #     Address family IPv4 Unicast: advertised and received
        #     Address family IPv6 Labeled-unicast: advertised and received
        #     Address family L2VPN EVPN: advertised and received
        p26 = re.compile(r'^Address +family +(?P<af_type>[\S\s]+): +(?P<state>[\S\s]+)$')

        #   Received 28148 messages, 0 notifications, 0 in queue
        p27 = re.compile(r'^Received +(?P<received_msg>\d+) +messages, '
                          '+(?P<received_notifications>\d+) +notifications, '
                          '+(?P<received_in_queue>\d+) +in +queue$')

        #   Sent 27671 messages, 0 notifications, 0 in queue
        p28 = re.compile(r'^Sent +(?P<sent_msg>\d+) +messages, '
                          '+(?P<sent_notifications>\d+) +notifications, '
                          '+(?P<sent_in_queue>\d+) +in +queue$')

        #   Minimum time between advertisement runs is 0 secs
        p29 = re.compile(r'^Minimum +time +between +advertisement +runs +is '
                          '+(?P<minimum_time_between_advertisement_runs>\d+) +secs$')

        #   Inbound message logging enabled, 3 messages buffered
        p30 = re.compile(r'^Inbound +message +logging +(?P<inbound_msg_logging>\w+), '
                          '+(?P<inbound_msg_buffered>\d+) +messages +buffered')

        #   Outbound message logging enabled, 3 messages buffered
        p31 = re.compile(r'^Outbound +message +logging +(?P<outbound_msg_logging>\w+), '
                          '+(?P<outbound_msg_buffered>\d+) +messages +buffered')

        #  For Address Family: IPv4 Unicast
        p32 = re.compile(r'^For +Address +Family: +(?P<af>[\S\s]+)$')

        #   BGP neighbor version 177
        p33 = re.compile(r'^BGP +neighbor +version +(?P<bgp_neighbor_version>\d+)$')

        #   Update group: 0.2 Filter-group: 0.1  No Refresh request being processed
        p34 = re.compile(r'^Update +group: +(?P<update_group>\S+) '
                          '+Filter-group: +(?P<filter_group>\S+) '
                          '+No +Refresh +request +being +processed$')

        #     Extended Nexthop Encoding: advertised and received
        p35 = re.compile(r'^Extended +Nexthop +Encoding: +(?P<state>[\S\s]+)$')

        #   Route refresh request: received 0, sent 0
        p36 = re.compile(r'^Route +refresh +request: +received '
                          '+(?P<received>\d+), +sent +(?P<sent>\d+)$')

        #   26 accepted prefixes, 11 are bestpaths
        p37 = re.compile(r'^(?P<accepted>\d+) +accepted +prefixes, '
                          '+(?P<best>\d+) +are +bestpaths$')

        #   Exact no. of prefixes denied : 0.
        p38 = re.compile(r'^Exact +no\. +of +prefixes +denied *: +(?P<num>\d+)\.$')

        #   Cumulative no. of prefixes denied: 0.
        p39 = re.compile(r'^Cumulative +no\. +of +prefixes +denied: +(?P<num>\d+)\.$')

        #     No policy: 0, Failed RT match: 45
        p39_1 = re.compile(r'^No +policy: +(?P<no_policy>\d+), '
                            '+Failed +RT +match: +(?P<failed_rt_match>\d+)$')

        #     By ORF policy: 0, By policy: 0
        p39_2 = re.compile(r'^By +ORF +policy: +(?P<by_orf_policy>\d+), '
                            '+By +policy: +(?P<by_policy>\d+)$')

        #   Prefix advertised 6, suppressed 0, withdrawn 2
        p40 = re.compile(r'^Prefix +advertised +(?P<prefix_advertised>\d+), '
                          '+suppressed +(?P<prefix_suppressed>\d+), '
                          '+withdrawn +(?P<prefix_withdrawn>\d+)$')

        #   Maximum prefixes allowed 1048576
        p41 = re.compile(r'^Maximum +prefixes +allowed +(?P<maximum_prefixes_allowed>\d+)$')

        #   Threshold for warning message 75%, restart interval 0 min
        p42 = re.compile(r'^Threshold +for +warning +message +(?P<warning_msg_threshold>\S+), '
                          '+restart +interval +(?P<restart_interval>\d+) +min$')

        #   AIGP is enabled
        p43 = re.compile(r'^AIGP +is +(?P<aigp>\S+)$')

        #   An EoR was received during read-only mode
        p44 = re.compile(r'^An EoR was received during read-only mode$')

        #   Last ack version 177, Last synced ack version 0
        p45 = re.compile(r'^Last +ack +version +(?P<last_ack_version>\d+), '
                          '+Last +synced +ack +version +(?P<last_synced_ack_version>\d+)$')

        #   Outstanding version objects: current 0, max 1, refresh 0
        p46 = re.compile(r'^Outstanding +version +objects: +current +(?P<current>\d+), '
                          '+max +(?P<max>\d+), +refresh +(?P<refresh>\d+)$')

        #   Additional-paths operation: None
        p47 = re.compile(r'^Additional-paths +operation: '
                          '+(?P<additional_paths_operation>\S+)$')

        #   Send Multicast Attributes
        p48 = re.compile(r'^Send +Multicast +Attributes$')

        #   Advertise routes with local-label via Unicast SAFI
        p49 = re.compile(r'^Advertise +routes +with +local-label +via +Unicast +SAFI$')

        #   Connections established 1; dropped 0
        p50 = re.compile(r'^Connections +established +(?P<established_connections>\d+); '
                          '+dropped +(?P<dropped_connections>\d+)$')

        #   Local host: 192.168.99.25, Local port: 18078, IF Handle: 0x00000000
        p51 = re.compile(r'^Local +host: +(?P<local_host>\S+), +Local +port: '
                          '+(?P<local_port>\d+), +IF +Handle: (?P<if_handle>\S+)$')

        #   Foreign host: 192.168.99.11, Foreign port: 179
        p52 = re.compile(r'^Foreign +host: +(?P<foreign_host>\S+), '
                          '+Foreign +port: +(?P<foreign_port>\d+)$')
        
        #   Last reset 00:00:00
        p53 = re.compile(r'^Last +reset +(?P<last_reset>\S+)$')
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                nbr_dict = result_dict.setdefault('neighbors', {}).setdefault(neighbor, {})
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict['bgp_state'] = group['bgp_state']
            ...
            if m:
                group = m.groupdict()
                nbr_dict.update({k: int(v) for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                sub_dict = nbr_dict.setdefault('last_write', {})
                sub_dict.update({k: (int(v) if v.isdigit() else v) for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                sub_dict = nbr_dict.setdefault('second_last_write', {})
                sub_dict.update({k: (int(v) if v.isdigit() else v) for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                sub_dict = nbr_dict.setdefault('last_write_reset', {})
                sub_dict.update({k: (int(v) if v.isdigit() else v) for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                sub_dict = nbr_dict.setdefault('second_last_write_reset', {})
                sub_dict.update({k: (int(v) if v.isdigit() else v) for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict['last_write_pulse_rcvd'] = group['time']
                nbr_dict['last_full_not_set_pulse_count'] = int(group['count'])
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict['last_write_pulse_rcvd_reset'] = group['reset']
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict.update({k: (False if 'not' in v else True) for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict['last_write_thread_event_reset'] = group['last']
                nbr_dict['second_last_write_thread_event_reset'] = group['second_last']
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict['last_ka_expiry'] = group['last']
                nbr_dict['second_last_ka_expiry'] = group['second_last']
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict['last_ka_error'] = group['last']
                nbr_dict['ka_not_sent'] = group['not_sent']
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict['last_ka_start'] = group['last']
                nbr_dict['second_last_ka_start'] = group['second_last']
                continue
            ...
            if m:
                group = m.groupdict()
                caps_dict = nbr_dict.setdefault('neighbor_capabilities', {})
                continue
            ...
            if m:
                group = m.groupdict()
                caps_dict.update({k: v for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                af_type = group['af_type'].lower().replace(" ", "_").replace("-", "_")
                caps_dict[af_type] = group['state']
                continue
            ...
            if m:
                group = m.groupdict()
                nbr_dict.update({k: (int(v) if v.isdigit() else v) for k, v in group.items()})
                continue
            ...
            if m:
                af = m.groupdict()['af'].lower()
                af_dict = nbr_dict.setdefault('address_family', {}).setdefault(af, {})
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict.update({k: float(v) for k, v in group.items()})
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict['extended_nexthop_encoding'] = group['state']
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict['route_refresh_request_received'] = int(group['received'])
                af_dict['route_refresh_request_sent'] = int(group['sent'])
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict['accepted_prefixes'] = int(group['accepted'])
                af_dict['bestpaths'] = int(group['best'])
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict['exact_no_prefixes_denied'] = int(group['num'])
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict['cummulative_no_prefixes_denied'] = int(group['num'])
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict['warning_msg_threshold'] = group['warning_msg_threshold']
                af_dict['restart_interval'] = int(group['restart_interval'])
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict['outstanding_version_objects_current'] = int(group['current'])
                af_dict['outstanding_version_objects_max'] = int(group['max'])
                af_dict['outstanding_version_objects_refresh'] = int(group['refresh'])
                continue
            ...
            if m:
                group = m.groupdict()
                af_dict.update({k: v for k, v in group.items()})
                continue
