...
class ShowL2vpnForwardingBridgeDomainMacAddress(MetaParser):
    ...
    def cli(self, output=None):
        ...
        # 1234.0002.0004 static  BE1.2                       N/A        N/A
        # N/A
        ...
        for idx, line in enumerate(lines):
            ...
            if not header_processed:
                ...
                mac, mac_type, learned_from, lc_learned, resync_age, mapped_to = _retrieve_fields(
                    line, field_indice)
    ...
    def cli(self, output=None):
        ...
        for idx, line in enumerate(lines):
            ...
            if not header_processed:
                ...
                interface, instance_id, state = _retrieve_fields(
                    line, field_indice)
                result['entries'].append({
                    'interface': interface,
...
class ShowL2vpnBridgeDomain(ShowL2vpnBridgeDomainSchema):
    ...
    def cli(self, output=None):
        ...
        # Bridge group: g1, bridge-domain: bd1, id: 0, state: up, ShgId: 0,
        # MSTi: 0
        p1 = re.compile(
            r'^Bridge +group: +(?P<bridge_group>\w+), +bridge\-domain: +'
            r'(?P<bridge_domain>\w+), +id: +(?P<id>\d+), +state: +(?P<state>\w+), +'
