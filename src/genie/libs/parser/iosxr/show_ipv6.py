...
class ShowIpv6NeighborsDetail(ShowIpv6NeighborsDetailSchema):
    ...
    def cli(self, output=None):
        ...
        p1 = re.compile(r'^(?P<ipv6_address>\S+)\s+(?P<age>\S+)\s+(?P<link_layer_add>\S+)\s+(?P<state>\S+)\s+(?P<interface>\S+)\s+'
        ...
        p2 = re.compile(r'^\[(?P<ipv6_address>([\w\s]+))\]([^\s]+) +(?P<age>\S+)\s+(?P<link_layer_add>\S+)\s+(?P<state>\S+)\s+(?P<interface>\S+)\s+(?P<location>\S+)\s+(?P<static>\S+)\s+(?P<dynamic>\S+)\s+(?P<sync>\S+)\s+(?P<serg_flags>\S+)$')
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                age = m.groupdict()['age']
                link_layer_add = m .groupdict()['link_layer_add']
                ...
                interface_dict = ret_dict.setdefault('interfaces', {}). \
                                          ...
                                          setdefault(ipv6_address, {})
                interface_dict['age'] = age
                interface_dict['link_layer_add'] = link_layer_add
                interface_dict['state'] = state
                interface_dict['location'] = location
                interface_dict['static'] = static
                interface_dict['dynamic'] = dynamic
                interface_dict['sync'] = sync
                interface_dict['serg_flags'] = serg_flags
                continue
