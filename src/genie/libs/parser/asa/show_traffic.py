...
class ShowTraffic(ShowTrafficSchema):
    ...
    def cli(self, output=None):
        ...
        # RegEx named capture group name assigned to key, matched text assigned to values
        # {'capture-group-name': 'capture group value'}
        ...
        for line in out.splitlines():
            ...
            if m:
                ...
                #print({groups['queue'] : { 'packets' : groups['packets']}})
                #instance_dict['queue'].update({ 'packets' : groups['packets']})      
                continue
            m = p4.match(line)
            if m:
                groups = m.groupdict()
